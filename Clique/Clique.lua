...
function Clique:Enable()
    ...
    self.numTalentGroups = C_Talent.GetNumTalentGroups()

    if self.db.char.switchSpec then
        self.silentProfile = true
        self.talentGroup = C_Talent.GetActiveTalentGroup()
		if not self.db.char["profile_"..self.talentGroup] then
			self.db.char["profile_"..self.talentGroup] = self.db.keys.profile
		end
		self.db:SetProfile(self.db.char["profile_"..self.talentGroup])
